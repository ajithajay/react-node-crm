 ...
 * The access token is handed to the iframe over postMessage once it signals ready (same-origin),
 * so it never appears in the iframe URL.
 */
export function RestPlaygroundPage() {
  const { schema } = useParams<{ schema: string }>();
  const iframeRef = useRef<HTMLIFrameElement>(null);
  const isValid = schema === 'core' || schema === 'metadata';
    ...
    function onMessage(event: MessageEvent): void {
      ...
      if ((event.data as { type?: string })?.type === 'playground-ready') {
        iframeRef.current?.contentWindow?.postMessage(
          { type: 'playground-config', schema, token: getAccessToken() ?? '', apiBaseUrl: getApiBaseUrl() },
