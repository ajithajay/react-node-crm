...
import type { MergeRecordsRequest, RecordListQuery } from '@saasly/shared';
import { AppError } from '../../lib/errors.js';
import { principalOf } from '../../lib/principal.js';
import * as recordService from './record.service.js';
import { findDuplicatesForRecord } from './duplicate.service.js';
import { mergeRecords } from './merge.service.js';
...
export async function duplicates(
  ...
  const result = await findDuplicatesForRecord(
    req.workspaceId!,
    principalOf(req),
    req.params.objectNamePlural,
    req.params.id,
...
export async function merge(
  req: Request<{ objectNamePlural: string }, unknown, MergeRecordsRequest>,
  res: Response,
): Promise<void> {
  const result = await mergeRecords(req.workspaceId!, principalOf(req), req.params.objectNamePlural, req.body);
  res.status(200).json(result);
}
